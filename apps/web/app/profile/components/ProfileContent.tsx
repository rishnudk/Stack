...
export function ProfileContent({ userId, isOwnProfile, initialTab = "work" }: ProfileContentProps) {
  const [activeTab, setActiveTab] = useState<"work" | "posts" | "resume" | "articles" | "projects" | "edit-profile">(initialTab);
  const [isAddProjectModalOpen, setIsAddProjectModalOpen] = useState(false);

  useEffect(() => {
    if (initialTab === "edit-profile") {
  ...
  return (
    <div className="w-full max-w-2xl mx-auto min-h-screen border-x border-neutral-800 bg-black text-white relative">
      ...
      {/* Edit Profile View */}
      {activeTab === "edit-profile" && user && isOwnProfile ? (
